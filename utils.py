...
def save_image1(tensor, output_dir, filename, denormalization=True, to_grayscale=True):
    """
    保存图像到OUTPUT_DIR
    :param tensor: [0-1]区间的图像张量，形状为(1, 3, h, w)或(3, h, w)
    :param output_dir: 输出路径
    ...
    if to_grayscale:
        image = image.convert('L') # Y=0.299⋅R+0.587⋅G+0.114⋅B
    image.save(os.path.join(output_dir, filename))